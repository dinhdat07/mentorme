...
export default function TutorCalendarPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CalendarSession[]>('/api/calendar/tutor', apiClient.get);
  const [actionError, setActionError] = useState<string | null>(null);
  const [loadingId, setLoadingId] = useState<string | null>(null);

  const t = {
    title: language === 'vi' ? 'Lịch dạy' : 'Teaching Calendar',
      ...
      <div className="p-8 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className={muted}>Upcoming sessions generated from class schedules.</p>
          </div>
          <button
            onClick={() => mutate()}
